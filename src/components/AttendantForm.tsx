    ...
    return (
      <div className="mt-4">
        ...
        {attendants.map((attendant, index) => (
          <div
            key={index}
            className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 border-b border-gray-200 pb-4 md:border-b-0 md:pb-0"
          >
            {/* Name Field */}
            <div className="space-y-2">
              <label
                htmlFor={`attendant_name_${index}`}
            ...
            <div className="space-y-2">
              <label
                htmlFor={`attendant_relation_${index}`}
                className="block text-sm font-medium text-gray-700 md:hidden"
              >
                Relation
              </label>
            ...
            {/* Phone Number Field with Conditional Delete Button */}
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <div className="flex-1">
                  <label
                    htmlFor={`attendant_phone_${index}`}
                    className="block text-sm font-medium text-gray-700 md:hidden"
                  >
                    Phone Number
                  </label>
                ...
                {attendants.length > 1 && (
                  <button
                    ...
                    className="text-red-600 hover:text-red-700 p-3 rounded-full hover:bg-red-50 transition-colors"
        ...
        {/* Add Another Attendant Button */}
        <button
          type="button"
          onClick={addAttendant}
          className="text-red-600 hover:text-red-700 font-medium flex items-center gap-2 py-2 px-4 rounded-lg hover:bg-red-50 transition-colors"
        >
          <svg
            className="w-5 h-5"
            fill="none"
